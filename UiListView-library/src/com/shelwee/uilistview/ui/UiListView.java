...
import com.shelwee.uilistview.R;
import com.shelwee.uilistview.model.BasicItem;
import com.shelwee.uilistview.model.IListItem;
import com.shelwee.uilistview.model.ViewItem;
...
public class UiListView extends LinearLayout {
	...
	public UiListView(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.UIListView, 0, 0);
		mListContainer = (LinearLayout) mMainContainer.findViewById(R.id.buttonsContainer);	
		
		mTextBold = a.getBoolean(R.styleable.UIListView_textBold, true);
		mAlignCenter = a.getBoolean(R.styleable.UIListView_alignCenter, false);
		mShowChevron = a.getBoolean(R.styleable.UIListView_showChevron, true);
		mSingleItem = a.getBoolean(R.styleable.UIListView_singleItem, false);
		
		if (mSingleItem) {
			mListContainer.setBackgroundDrawable(null);
		}
	...
	private void setupBasicItem(View view, BasicItem item, int index) {
		...
		if (mTextBold) {
			((TextView) view.findViewById(R.id.title)).setTypeface(Typeface.defaultFromStyle(Typeface.BOLD));
		}
		if (mAlignCenter) {
